...
public class LendReturnServiceImpl implements LendReturnService {
	...
	public Map<String, Object> returnCommit(Map<String, Object> paramMap) {
		...
		double t1 = voteFeeAmt.add(transitAmtTotal).setScale(1, BigDecimal.ROUND_HALF_UP).doubleValue();//Double.parseDouble(voteFeeAmt) + Double.parseDouble(transitAmtTotal);
		double t2 = totalAmt.doubleValue();
		if(t1 != t2) {
			log.error("t1: " + t1 + "t2:" + t2);
